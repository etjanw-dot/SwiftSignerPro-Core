...
struct SigningOptionsView: View {
    ...
    var body: some View {
        ...
        NBSection(.localized("Orientation")) {
            _toggle(.localized("Force Portrait"),
                    systemImage: "iphone",
                    isOn: $options.forcePortrait,
                    temporaryValue: temporaryOptions?.forcePortrait
            )
            
            _toggle(.localized("Force Landscape"),
                    systemImage: "iphone.landscape",
                    isOn: $options.forceLandscape,
                    temporaryValue: temporaryOptions?.forceLandscape
            )
            
            _toggle(.localized("Force Multitasking"),
                    systemImage: "rectangle.split.2x1",
                    isOn: $options.forceMultitasking,
                    temporaryValue: temporaryOptions?.forceMultitasking
            )
        } footer: {
            Text(.localized("Control app orientation and multitasking behavior"))
        }
        
        NBSection(.localized("Security & Network")) {
            _toggle(.localized("Allow Arbitrary Loads"),
                    systemImage: "network.slash",
                    isOn: $options.allowArbitraryLoads,
                    temporaryValue: temporaryOptions?.allowArbitraryLoads
            )
            
            _toggle(.localized("Disable File Protection"),
                    systemImage: "lock.open",
                    isOn: $options.disableFileProtection,
                    temporaryValue: temporaryOptions?.disableFileProtection
            )
            
            _toggle(.localized("Force Dark Keyboard"),
                    systemImage: "keyboard",
                    isOn: $options.forceDarkKeyboard,
                    temporaryValue: temporaryOptions?.forceDarkKeyboard
            )
        } footer: {
            Text(.localized("Allow Arbitrary Loads disables App Transport Security (ATS) restrictions"))
        }
        
        NBSection(.localized("Additional Removal")) {
            _toggle(.localized("Remove Extensions"),
                    systemImage: "puzzlepiece.extension",
                    isOn: $options.removeExtensions,
                    temporaryValue: temporaryOptions?.removeExtensions
            )
            
            _toggle(.localized("Remove Plugins"),
                    systemImage: "powerplug",
                    isOn: $options.removePlugins,
                    temporaryValue: temporaryOptions?.removePlugins
            )
            
            _toggle(.localized("Remove App Clips"),
                    systemImage: "appclip",
                    isOn: $options.removeAppClips,
                    temporaryValue: temporaryOptions?.removeAppClips
            )
        } footer: {
            Text(.localized("Remove additional components from the app bundle"))
        }
        
        NBSection(.localized("Background")) {
            _toggle(.localized("Enable Background Refresh"),
                    systemImage: "arrow.clockwise.circle",
                    isOn: $options.enableBackgroundRefresh,
                    temporaryValue: temporaryOptions?.enableBackgroundRefresh
            )
            
            _toggle(.localized("Background Audio"),
                    systemImage: "speaker.wave.2",
                    isOn: $options.backgroundAudio,
                    temporaryValue: temporaryOptions?.backgroundAudio
            )
        } footer: {
            Text(.localized("Control background execution modes"))
        }
        
        NBSection(.localized("Signing Method")) {
            _picker(.localized("Method"),
                    systemImage: "signature",
                    selection: $options.signingMethod,
                    values: Options.signingMethodValues,
                    id: \.description
            )
        } footer: {
            Text(.localized("Standard: Normal signing with Zsign\nDirect: Direct signing method\nLdid: Use ldid for signing"))
        }
    }
