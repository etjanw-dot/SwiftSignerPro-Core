...
private extension DownloaderView {
    ...
    var content: some View {
        downloadsList
            .overlay {
                if isLoading {
                    // Show skeleton loading that matches the downloads list layout
                    SkeletonListView()
                } else if downloadManager.downloadItems.isEmpty {
                    if #available(iOS 17, *) {
            ...
            .onAppear {
                // Mark loading as complete after items are loaded
