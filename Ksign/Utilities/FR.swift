	...
	static func handlePackageFile(
		...
		Task.detached {
			...
			do {
				...
				// Check and perform auto-sign if enabled
				if AutoSignManager.shared.isEnabled {
					let appUUID = handler.uuid
					await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
						AutoSignManager.shared.performAutoSign(forAppWithUUID: appUUID) { _ in
							continuation.resume()
						}
					}
				}
